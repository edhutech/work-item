...
**F2A current-status supersession:** The earlier handoff value `23439` and its two-range execution instruction are historical. The authoritative completed search-log entry records the Scopus Web retrieval snapshot as `23444` (`17216 + 6228`) after observed database drift. No unique corpus count is derived, and no deduplication or screening has started.
