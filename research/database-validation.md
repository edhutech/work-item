...
The utility now explicitly defaults to `view=COMPLETE` and `count=25`. This conservative limit is retained because the protocol requires an abstract when legally and technically available, and Scopus `COMPLETE` supports at most 25 records/request. `view=STANDARD` remains available at up to 200 records/request only when a separately documented reduced metadata contract is acceptable; it must not silently replace the complete metadata representation.
